...
parser.add_argument('--use_tmp', type=bool_flag, default=True)
...
def main():
    ...
    if not os.path.isfile(result_path):
        ...
        results['test'] = {key:[] for key in ['loss', 'pearson', 'pred', 'true', 'vector' 'raw_src', 'raw_ref', 'raw_hyp', 'sid', 'vector']}
    else:
    ...
    if 'batch={}'.format(args.batch_size) not in results['valid'][args.optimizer]:
        results['train'][args.optimizer]['batch={}'.format(args.batch_size) ] =  [{key:[] for key in ['loss', 'pearson', 'pred', 'true', 'raw_src', 'raw_ref', 'raw_hyp', 'sid']} 
                                                                               for _ in range(args.trial_times)]
        results['valid'][args.optimizer]['batch={}'.format(args.batch_size) ] =  [{key:[] for key in ['loss', 'pearson', 'pred', 'true', 'raw_src', 'raw_ref', 'raw_hyp', 'sid']} 
                                                                               for _ in range(args.trial_times)] 
